 ...
 */
final class StructureCollection implements \Countable, \JsonSerializable
{
    /* @var \Mnemesong\Structure\Structure[] $objects */
    ...
    {
        Assert::allIsInstanceOf($objects, Structure::class);
        $this->objects = array_values($objects);
    }

    /**
     * @param \Mnemesong\Structure\Structure $object
     * @return self
     */
    public function withNewOneItem(Structure $object): self
    {
        Assert::isInstanceOf($object, Structure::class,
            "Added items should be instance of Structure");
        $clone = clone $this;
    ...
    /**
     * @param \Mnemesong\Structure\Structure[] $objects
     * @return self
     */
     ...
     * @return int|null
     */
    protected function getIndex(Structure $object): ?int
    {
        $filtered = array_filter($this->objects, function (Structure $innerObject) use ($object) {
     ...
     * @return int[]
     */
    protected function getAllIndexes(Structure $object): array
    {
        $filtered = array_filter($this->objects, function (Structure $innerObject) use ($object) {
            return $innerObject == $object;
        });
     ...
     * @return self
     */
    public function withoutObjectsLike(Structure $object, int $limit = 0): self
    {
        $arr = $this->objects;
        $i = $limit;
        if($limit < 0) {
    ...
    public function reworkedBy(callable $callbackFunction): self
    {
        ...
        $checkFilter = array_filter($newArray, function ($item) {
            return !is_a($item, Structure::class);
        });
    ...
    /**
     * @return \Mnemesong\Structure\Structure[]
     */
     ...
     * @return \Mnemesong\Structure\Structure
     */
    public function getFirstAsserted(): Structure
    {
        if($this->count() === 0) {
            throw new \RuntimeException('Error: try to get first element from empty Structures collection');
        }
     ...
     * @return \Mnemesong\Structure\Structure
     */
    public function getLastAsserted(): Structure
    {
        if($this->count() === 0) {
            throw new \RuntimeException('Error: try to get last element from empty Structures collection');
        }
    ...
    /**
     * @return \Mnemesong\Structure\Structure|null
     */
    public function getFirstOrNull(): ?Structure
    {
        if($this->count() > 0) {
     ...
     * @return \Mnemesong\Structure\Structure|null
     */
    public function getLastOrNull(): ?Structure
    {
        if($this->count() > 0) {
